...
with open('LocationHistory_New.json') as json_data:
    data = json.load(json_data)

writer = csv.writer(open("AllPlaces_New.csv", "wb"))
...
def coordUnique(seq, idfun=None):
    ...
    for x in locations:
        ...
        #Prevent duplicates
        roundedLat = '%.6f' % (latitude)    #Rounds to 4 decimals
        roundedLon = '%.6f' % (longitude)
...
'''
def coordNearby(seq, idfun=None):
    # order preserving
    if idfun is None:
        def idfun(x): return x
    seen = {}
...
listCoords = coordUnique(json_data) #Find unique coordinates to 4 decimal places of my json
#nearbyCoords = coordNearby(listCoords)__author__ = 'glassman'
